...
/**
 * Scraper for Wellfound (formerly AngelList Talent) - Startup jobs
 * Disabled in favor of EnhancedWellfoundScraper
 */
// @Component
public class WellfoundScraper implements PortalScraper {
    ...
    @Value("${job.portals.wellfound.requestDelay:2000}")
    ...
    public List<JobListing> scrapeJobs() throws IOException {
        ...
        for (int page = 1; page <= maxPages; page++) {
            try {
                ...
                Elements jobCards = doc.select(".job-listing, [data-test='JobSearchResult']");
                ...
                System.err.println("Error fetching Wellfound page " + page + ": " + e.getMessage());
                throw e;
            }
        }
    ...
    private String buildSearchUrl(int page) {
        try {
            ...
            String url = BASE_URL + "/role/r/" + encodedQuery.replace("+", "-");
            
            // Add location filter
            url += "/l/" + encodedLocation.replace("+", "-");
            ...
            return BASE_URL + "/role/r/software-engineer/l/India?experience=entry_level";
        }
    }
    ...
    private JobListing parseJobCard(Element card) {
        ...
        Element titleElement = card.selectFirst(".job-title, [data-test='JobTitle'], h2 a");
        if (titleElement != null) {
            job.setTitle(titleElement.text().trim());
        }
        
        Element companyElement = card.selectFirst(".company-name, [data-test='CompanyName']");
        ...
        if (descElement != null) {
            ...
            job.setDescription("Entry-level startup position from Wellfound");
        }
        
        Element linkElement = card.selectFirst("a[href*='/jobs/'], a[href*='/company/']");
        if (linkElement != null) {
