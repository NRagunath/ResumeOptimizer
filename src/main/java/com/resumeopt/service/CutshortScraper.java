...
/**
 * Scraper for Cutshort job portal (Tech jobs platform)
 * Disabled in favor of EnhancedCutshortScraper
 */
// @Component
public class CutshortScraper implements PortalScraper {
    ...
    public List<JobListing> scrapeJobs() throws IOException {
        ...
        for (int page = 1; page <= maxPages; page++) {
            try {
                ...
                Elements jobCards = doc.select(".job-card, .opportunity-card");
                ...
                System.err.println("Error fetching Cutshort page " + page + ": " + e.getMessage());
                throw e;
            }
        }
    ...
    private String buildSearchUrl(int page) {
        try {
            ...
            return BASE_URL + "/jobs/software-engineer?experience=0-2";
        }
    }
    ...
    private JobListing parseJobCard(Element card) {
        ...
        Element titleElement = card.selectFirst(".job-title, h3 a, .title");
        if (titleElement != null) {
            job.setTitle(titleElement.text().trim());
        }
        
        Element companyElement = card.selectFirst(".company-name, .company");
        ...
        if (descElement != null) {
            ...
            job.setDescription("Tech position from Cutshort");
        }
        
        Element linkElement = card.selectFirst("a[href*='/job/'], a.job-link");
        if (linkElement != null) {
