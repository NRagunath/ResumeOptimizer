...
/**
 * Scraper for Shine job portal (India-focused)
 * Disabled in favor of EnhancedShineScraper
 */
// @Component
public class ShineScraper implements PortalScraper {
    ...
    @Value("${job.portals.shine.requestDelay:2000}")
    private long requestDelay;
    
    @Value("${job.portals.shine.maxPages:3}")
    private int maxPages;
    
    @Value("${job.portals.shine.experienceMax:2}")
    ...
    public List<JobListing> scrapeJobs() throws IOException {
        ...
        for (int page = 1; page <= maxPages; page++) {
            try {
                ...
                Elements jobCards = doc.select(".jobCard, .job_listing");
                ...
                System.err.println("Error fetching Shine page " + page + ": " + e.getMessage());
                throw e;
            }
        }
    ...
    private String buildSearchUrl(int page) {
        try {
            ...
            return BASE_URL + "/job-search/software-engineer-jobs";
        }
    }
    ...
    private JobListing parseJobCard(Element card) {
        ...
        Element titleElement = card.selectFirst(".jobCard_pReplaceH2, .job_title a, h2 a");
        if (titleElement != null) {
            job.setTitle(titleElement.text().trim());
        }
        
        Element companyElement = card.selectFirst(".jobCard_companyName, .company_name");
        if (companyElement != null) {
            job.setCompany(companyElement.text().trim());
        }
        
        Element descElement = card.selectFirst(".jobCard_jobDescription, .job_description");
        if (descElement != null) {
            job.setDescription(descElement.text().trim());
        } else {
            job.setDescription("Entry-level position from Shine (0-2 years experience)");
        }
        
        Element linkElement = card.selectFirst("a[href*='/job-detail/'], .job_title a");
        if (linkElement != null) {
